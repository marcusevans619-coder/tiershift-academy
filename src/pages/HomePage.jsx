...
const teamFeatures = [
  {icon:"Ã°Å¸â€œâ€¹",title:"Assign Learning Paths",desc:"Push specific tracks to individual techs or your entire team. New hire? Assign Tier 1 to Tier 2 on day one.",color:"#00e5ff"},
  {icon:"Ã°Å¸â€œË†",title:"Real-Time Progress Dashboard",desc:"See exactly where every tech is Ã¢â‚¬â€ hours logged, labs completed, certs earned. No more chasing people for updates.",color:"#a78bfa"},
  {icon:"Ã°Å¸Å½Â¯",title:"Skill Gap Analysis",desc:"Identify which techs need what training before problems surface. Get ahead of knowledge gaps proactively.",color:"#34d399"},
  {icon:"Ã°Å¸Ââ€ ",title:"Team Leaderboards",desc:"Friendly competition drives completion. See who is leading on certifications and lab completions.",color:"#fbbf24"},
  {icon:"Ã°Å¸â€œâ€ž",title:"Completion Reports",desc:"Export certification reports for HR reviews, compliance audits, or client-facing proof of team competency.",color:"#fb7185"},
  {icon:"Ã°Å¸â€Â§",title:"Custom Content",desc:"Work with us to build tracks specific to your tools, ticketing system, and internal processes.",color:"#38bdf8"},
];
...
const faqs = [
  ...
  {q:"Is this better than CBT Nuggets or Udemy for Business?",a:"TierShift is built specifically for IT support teams, not general learners. Labs mirror real helpdesk scenarios Ã¢â‚¬â€ ticket systems, terminal commands, network configs Ã¢â‚¬â€ not generic tech content."},
...
const terminalLines = [
  ...
  {text:"Ã¢Å“â€œ Lab complete! +250 XP earned",color:"#fbbf24"},
];
...
export default function HomePage({onGetStarted, onSignIn}) {
  ...
  return (
    <div style={{backgroundColor:"#060a12",color:"#e8edf5",fontFamily:"Outfit,sans-serif",minHeight:"100vh",overflowX:"hidden"}}>
          ...
          <div className="fadeUp">
            ...
            {activeTab==="teams"?(
              <div>
                ...
                <p style={{fontSize:16,color:"#94a3b8",lineHeight:1.75,marginBottom:20,maxWidth:480}}>Assign tracks, track progress, and certify your techs Ã¢â‚¬â€ all from one dashboard. Built for IT managers who need results, not just completion percentages.</p>
                <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:32}}>
                  {["Assign learning paths to specific techs or your whole team","Real-time progress dashboard Ã¢â‚¬â€ see who needs a push","Auto-graded labs and certification exams","Exportable completion reports for HR and compliance"].map((item,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:10,fontSize:13,color:"#94a3b8"}}><span style={{color:"#34d399"}}>Ã¢Å“â€œ</span>{item}</div>))}
                </div>
              </div>
            ):(
              <div>
                ...
                <p style={{fontSize:16,color:"#94a3b8",lineHeight:1.75,marginBottom:20,maxWidth:480}}>Hands-on labs, certification exams, and career tracks built for IT support professionals ready to level up. No fluff Ã¢â‚¬â€ just real skills.</p>
                <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:32}}>
                  {["60+ hands-on terminal, ticket, and config labs","9 career tracks from Tier 1 to Security Engineer","8 certification exams with downloadable credentials","Track your progress with a real-time dashboard"].map((item,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:10,fontSize:13,color:"#94a3b8"}}><span style={{color:"#34d399"}}>Ã¢Å“â€œ</span>{item}</div>))}
                </div>
              </div>
            )}
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              <button onClick={()=>setShowDemoModal(true)} className="bp" style={{padding:"13px 28px",background:"linear-gradient(135deg,#00e5ff,#a78bfa)",border:"none",borderRadius:10,color:"#060a12",fontSize:15,fontWeight:800,cursor:"pointer",transition:"all .2s"}}>{activeTab==="teams"?"Schedule a Demo Ã¢â€ â€™":"Start Free Today Ã¢â€ â€™"}</button>
          ...
          {!isMobile && <div style={{animation:"float 4s ease-in-out infinite", display: isMobile ? "none" : "block"}}>
            <div style={{background:"#111827",border:"1px solid #1e2d40",borderRadius:16,overflow:"hidden",boxShadow:"0 24px 80px rgba(0,0,0,.6),0 0 40px #00e5ff15"}}>
              <div style={{padding:"10px 14px",background:"#0d1520",borderBottom:"1px solid #1e2d40",display:"flex",alignItems:"center",gap:7}}>
                ...
                <span style={{marginLeft:8,fontSize:11,color:"#64748b",fontFamily:"monospace"}}>lab-terminal Ã¢â‚¬â€ Network Fundamentals</span>
              </div>
              ...
              <div style={{margin:"0 16px 16px",padding:"12px 14px",background:"#34d39910",border:"1px solid #34d39930",borderRadius:10}}>
                <div style={{fontSize:11,color:"#34d399",fontWeight:700,marginBottom:6,fontFamily:"monospace"}}>MANAGER VIEW Ã¢â‚¬â€ LIVE</div>
      ...
      <section ref={statsRef} style={{padding:"60px 48px",borderTop:"1px solid #1e2d40",borderBottom:"1px solid #1e2d40",background:"#0d1520"}}>
      ...
      <section style={{padding:"80px 48px",background:"#0d1520"}}>
        <div style={{maxWidth:960,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:24,position:"relative"}}>
            ...
            {[
              {step:"01",icon:"Ã°Å¸â€˜Â¤",title:"Admin Setup",role:"You (Manager)",desc:"Create your team, invite techs, assign the right learning paths to each person."},
              {step:"02",icon:"Ã°Å¸â€“Â¥Ã¯Â¸Â",title:"Techs Train",role:"Your Team",desc:"Each tech works through labs, scenarios, and exams at their own pace."},
              {step:"03",icon:"Ã°Å¸â€œÅ ",title:"You Monitor",role:"You (Manager)",desc:"Watch progress in real time. See who is ahead, who is falling behind, and intervene early."},
              {step:"04",icon:"Ã°Å¸Ââ€ ",title:"Certs Earned",role:"Everyone Wins",desc:"Techs earn verified credentials. You get a more capable team."},
            ].map((s,i)=>(
          ...
          <div style={{background:"#111827",border:"1px solid #1e2d40",borderRadius:16,overflow:"hidden"}}>
            ...
            {comparisonData.map((row,i)=>(
              <div key={i} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"11px 20px",borderBottom:i<comparisonData.length-1?"1px solid #1e2d40":"none",background:i%2===0?"transparent":"#0d152060"}}>
                ...
                {[row.ts,row.youtube,row.cbt].map((v,j)=>(<div key={j} style={{textAlign:"center",fontSize:16}}>{v?<span style={{color:"#34d399"}}>Ã¢Å“â€œ</span>:<span style={{color:"#1e2d40"}}>Ã¢â‚¬â€</span>}</div>))}
              </div>
            ))}
          </div>}
        </div>
      ...
      <section id="demo-section" style={{padding:"80px 48px",background:"#0d1520"}}>
        <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
          <div style={{padding:"56px 44px",background:"linear-gradient(135deg,#00e5ff10,#a78bfa10)",border:"1px solid #00e5ff30",borderRadius:22,position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",justifyContent:"center",gap:20,flexWrap:"wrap"}}>
              {["No credit card required","Setup in under 10 minutes","Cancel anytime"].map((item,i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:5,fontSize:12,color:"#64748b"}}><span style={{color:"#34d399"}}>Ã¢Å“â€œ</span>{item}</div>))}
            </div>
          </div>}
        </div>
      </section>
      ...
      <footer style={{padding:"24px 48px",borderTop:"1px solid #1e2d40",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontSize:12,color:"#64748b"}}>TierShift Academy Ã‚Â© 2025</span>
        </div>
